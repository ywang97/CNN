...
def train_neural_counter(layers,data,image_size=1000,tsize=1000,loss_func='mse'):
    ...
    history = LossHistory()    
    model.fit(X_train, Y_train, epochs=epochs, batch_size=batch, validation_data=(X_val, Y_val),callbacks=[history], verbose=False)
...
def get_MNIST_data(shift=0,scale=False):
...
def part3(dataset,cl,epoch,trials,verbose,display,split):
    for layers in archs(cl):
        run_keras_2d(str(dataset),layers,epoch,trials=trials,verbose=verbose,\
                     display=display,split=split)
        
def part4(tsize,imsize,trials):
    data=get_image_data_1d(tsize,imsize,0.1)
    
    for trial in range(trials):
        layer1=Conv1D(filters=1, kernel_size=2, kernel_regularizer=filter_reg, strides=1,use_bias=False,activation='relu',input_shape=(imsize,1),padding='same')
        ...
        print(np.mean(model.layers[-1].get_weights()[0]))

train, validation = get_MNIST_data(scale=True,shift=20)
print('con')
...
layer1=Conv2D(filters=32,kernel_size=(3,3),activation='relu',input_shape=(48,48,1))
...
layer7=Dropout(rate=0.5)
layer8=Dense(units=10,activation='softmax')
layers=[layer1,layer2,layer3,layer4,layer5,layer6,layer7,layer8]
run_keras_cnn_mnist(train,validation,layers,1,split=0.1,verbose=False,trials=5)
